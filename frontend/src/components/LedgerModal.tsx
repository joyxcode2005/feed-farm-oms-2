...
export default function LedgerModal({ material, onClose }: Props) {
  ...
  const fetchLedger = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/raw-materials/${material.id}/ledger`, {
        params,
      });
              ...
              {loading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    ...
                    <td className="px-6 py-4">
                      <div className="h-6 w-20 bg-zinc-100 dark:bg-zinc-800 rounded"></div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="h-4 w-40 bg-zinc-100 dark:bg-zinc-800 rounded"></div>
                    </td>
                  </tr>
                ))
              ) : ledger.length > 0 ? (
                ledger.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold tracking-tight border shadow-sm ${
                          item.type === "IN"
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        {item.notes && (
                          <span className="text-zinc-900 dark:text-zinc-200 line-clamp-1 text-sm">
                            {item.notes}
                          </span>
                        )}
                        {item.referenceId && (
