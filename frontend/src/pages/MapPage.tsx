...
import { DISTRICT_STYLES } from "../config";
...
export default function MapPage() {
  ...
  const { mapFeatures, pathGenerator } = useMemo(() => {
    ...
    return { mapFeatures: geoJson.type === "FeatureCollection" ? geoJson.features : [geoJson], pathGenerator };
  }, []);
  ...
  const fetchCustomersByDistrict = async (district: string) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/customers?district=${district}`,
        {
          credentials: "include",
        }
      );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-4xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight mb-3">
            ...
            <span className="text-indigo-600 dark:text-indigo-400">Map</span>
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl mx-auto">
            Interactive visualization of customer distribution across West
            Bengal.
          </p>
        </div>
        ...
        <div className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 overflow-hidden border border-slate-100 dark:border-slate-800 p-8">
          ...
          >
            {mapFeatures.map((feature: any) => {
              ...
              const colorClass =
                DISTRICT_STYLES[districtName] || "fill-slate-200"; // Fallback color
                ...
                >
                  <path
                    ...
                    className={`
                      ...
                      ${
                        isSelected
                          ? "brightness-90 stroke-[1.5] -translate-y-0.5 drop-shadow-xl z-10"
                          : "opacity-90 hover:opacity-100 hover:-translate-y-0.5 hover:drop-shadow-md"
                      }
                    `}
...
function DistrictModal({
  ...
  // Kept 'any' for brevity, use full types from previous response if needed
  return (
