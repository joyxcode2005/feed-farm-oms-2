...
export async function generateDailySnapshots() {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const material of rawMaterials) {
      ...
      const totalIn = txns.filter(t => t.type === "IN").reduce((acc, t) => acc + t.quantity, 0);
      const totalOut = txns.filter(t => t.type === "OUT").reduce((acc, t) => acc + t.quantity, 0);
      ...
      await tx.rawMaterialDailySnapshot.upsert({
        ...
        update: { openingStockKg: openingStock, totalInKg: totalIn, totalOutKg: totalOut, closingStockKg: closingStock },
        create: { rawMaterialId: material.id, date: startDate, openingStockKg: openingStock, totalInKg: totalIn, totalOutKg: totalOut, closingStockKg: closingStock },
      });
    }

    // 2. Finished Feed Snapshots
    const feedCategories = await tx.feedCategory.findMany();
    for (const category of feedCategories) {
      ...
      const inBags = txns.filter(t => t.type === "PRODUCTION_IN").reduce((acc, t) => acc + t.quantityBags, 0);
      const outBags = txns.filter(t => t.type === "SALE_OUT").reduce((acc, t) => acc + t.quantityBags, 0);
      ...
      await tx.finishedFeedDailySnapshot.upsert({
        ...
        create: { feedCategoryId: category.id, date: startDate, openingBags, inBags, outBags, closingBags },
      });
    }
  });
}