...
/**
 * Route to get the global summary of raw materials (Must be before /:id)
 */
router.get("/summary", async (req: Request, res: Response) => {
  try {
    const { from, to } = req.query;
    if (!from || !to) return res.status(400).json({ success: false, message: "Date range required" });
    const summary = await getRawMaterialsSummaryDB(new Date(from as string), new Date(to as string));
